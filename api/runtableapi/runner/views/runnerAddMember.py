...
class RunnerAddMemberView(APIView):
     def post(self, request, *arg, **kwargs):
        ...
        running = Running.objects.values_list('day', 'date')
        running = list( dict.fromkeys(running) )
 
        for i in running:
            Running.objects.create(day=i[0], distant=0, date=i[1], runner= Runner.objects.get(runner_id=Runner.objects.last().runner_id))
